...
class S3Repository(private val context: Context) {
    ...
    companion object {
        private const val BUCKET_NAME = "YOUR_BUCKET_NAME"
                ...
                override fun onStateChanged(id: Int, state: TransferState) {
                    if (state == TransferState.COMPLETED) {
                        try {
                            ...
                            // Store the URL and the object key in Firestore
                            // We save both because we'll need the key for later operations
                            val resultMap = mapOf(
                                "url" to presignedUrl,
    ...
    suspend fun deleteProfilePhoto(fileUrl: String): Boolean = withContext(Dispatchers.IO) {
        try {
            // Since we're using presigned URLs, we need to extract the object key
            // This assumes we also store the object key in Firestore along with the URL
            // If we can't get the object key directly, we may need to query Firestore

            // For now, let's extract it from the URL (this is a simplification)
